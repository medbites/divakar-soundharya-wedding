...
og-image.jpg            the picture WhatsApp shows when the link is shared (upload it next to index.html)
...
1. **Link preview picture (WhatsApp, iMessage).** See the next section. You must upload `og-image.jpg` next to `index.html`.
...
## The WhatsApp link preview (thumbnail, title and description)

When someone shares the link, WhatsApp shows a card: the picture in `og-image.jpg` (you and Sankar with the names and dates),
the title "Mahati & Sankar | Wedding Invitation" and a short description. This is set in the first lines of `index.html`.

For the picture to appear, **`og-image.jpg` must be uploaded into the same folder as `index.html`**, so that this opens in a browser:

`https://www.jobosskystudios.online/mswedding/og-image.jpg`

Then:
1. Upload the new `index.html` and `og-image.jpg` (both are in this zip; `index.html` alone is not enough for the picture).
2. WhatsApp remembers the old, picture-less card for a while. To see the new one straight away, share the link with a
   dummy ending, for example `https://www.jobosskystudios.online/mswedding?v=2`. Use a new number each time you re-test.
3. To check what WhatsApp will see, paste the link into Facebook's Sharing Debugger (developers.facebook.com/tools/debug)
   and press "Scrape Again".

If your address is different from the one above, edit the lines starting `og:url`, `og:image`, `og:image:secure_url`,
`twitter:image` and `canonical` at the top of `index.html`. The picture is 1200 x 630 pixels and about 100 KB, which is the
size WhatsApp likes. The site itself opens on the curtain almost immediately, then finishes loading the photos and music behind it.
